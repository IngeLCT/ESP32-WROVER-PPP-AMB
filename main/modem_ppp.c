...
static void on_ip_event(void *arg, esp_event_base_t base, int32_t id, void *data) {
    if (id == IP_EVENT_PPP_GOT_IP) {
        ...
        ESP_LOGI(TAG, "PPP UP  ip=" IPSTR " gw=" IPSTR,
                                   ...
                                   esp_modem_dce_t **out_dce)
{
    ...
    bool dns_ok = (dmain.ip.u_addr.ip4.addr != 0) || (dbackup.ip.u_addr.ip4.addr != 0);
    if (!dns_ok) {
        ESP_LOGW(TAG, "DNS vacío, asignando 1.1.1.1 y 8.8.8.8");
        esp_netif_dns_info_t dns1 = { .ip.u_addr.ip4.addr = inet_addr("1.1.1.1") };
        esp_netif_set_dns_info(ppp, ESP_NETIF_DNS_MAIN, &dns1);
        esp_netif_dns_info_t dns2 = { .ip.u_addr.ip4.addr = inet_addr("8.8.8.8") };
                               ...
                               int *tac, int *cid)
{
    ...
    char rsp[256] = {0};
    if (esp_modem_at(dce, "AT+CPSI?\r", rsp, sizeof(rsp), 3000) != ESP_OK) {
        ESP_LOGE(TAG, "AT+CPSI? falló");
        return false;
    }

    // Ejemplo de respuesta: +CPSI: LTE,Online,334,20,562,44012843,...
    int _mcc=0,_mnc=0,_tac=0,_cid=0;
    if (sscanf(rsp, "%*[^,],%*[^,],%d,%d,%d,%d", &_mcc,&_mnc,&_tac,&_cid) == 4) {
        if (mcc) *mcc=_mcc;
        if (mnc) *mnc=_mnc;
        if (tac) *tac=_tac;
        if (cid) *cid=_cid;
        ESP_LOGI(TAG, "CPSI: MCC=%d MNC=%d TAC=%d CID=%d", _mcc,_mnc,_tac,_cid);
        return true;
    }

    ESP_LOGW(TAG, "No se pudo parsear AT+CPSI? rsp='%s'", rsp);
    return false;
}
